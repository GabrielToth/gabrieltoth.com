...
function verifyMigrations() {
    console.log('🔍 Verifying Supabase DB Migrations in preview/CI environment...');
    ...
    for (const file of files) {
        ...
        if (!match) {
            console.error(`❌ Invalid migration filename format: "${file}". Must start with timestamp format (YYYYMMDD_name.sql).`);
            errors++;
            continue;
        }
        ...
        if (timestamp < lastTimestamp) {
            console.warn(`⚠️ Warning: Migration timestamp ordering out of sequence for file: "${file}".`);
        }
    ...
    if (errors > 0) {
        console.error(`❌ DB Migration verification failed with ${errors} error(s).`);
        process.exit(1);
    } else {
