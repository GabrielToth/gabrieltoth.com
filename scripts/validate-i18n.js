...
function validateFile(fileName) {
    ...
    for (const lang of LANGUAGES) {
        ...
        try {
            ...
            console.error(
                `❌ Failed to parse ${lang}/${fileName}: ${error.message}`
            )
            hasErrors = true
            return
        }
    ...
    for (const lang of LANGUAGES) {
        ...
        for (const key of refKeySet) {
            ...
            if (typeof refValue !== typeof langValue) {
                console.error(
                    `❌ [${lang}] Type mismatch for key "${key}": ${REFERENCE_LANG} has ${typeof refValue}, ${lang} has ${typeof langValue}`
                )
                hasErrors = true
                continue
            }
            ...
            if (typeof refValue === "string") {
                ...
                if (
                    refParams.length !== langParams.length ||
                    refParams.join(",") !== langParams.join(",")
                ) {
                    console.error(
                        `❌ [${lang}] Parameter mismatch for key "${key}":`
                    )
                    console.error(
                        `   ${REFERENCE_LANG}: ${refParams.join(", ") || "(none)"}`
                    )
                    console.error(
                        `   ${lang}: ${langParams.join(", ") || "(none)"}`
                    )
                    hasErrors = true
                }
            }
