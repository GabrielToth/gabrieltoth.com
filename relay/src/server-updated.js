...
function getStatus(connections) {
    ...
    const twitchStatus = values.some(c => c.twitchConnected)
        ? "connected"
        : "disconnected"
    const kickStatus = values.some(c => c.kickConnected)
        ? "connected"
        : "disconnected"
    const youtubeStatus = values.some(c => c.youtubeConnected)
        ? "connected"
        : "disconnected"
    return {
        status: "online",
...
const server = createServer((req, res) => {
    ...
    const cacheMatch = url.pathname.match(
        /^\/cache\/(avatars|thumbnails)\/(twitch|kick)\/(.+)$/
    )
    if (cacheMatch) {
        ...
        serveCachedImage(
            ...
            platform,
            name,
            type === "thumbnails" ? "thumbnail" : "avatar"
        )
        return
    }
...
const wss = new WebSocketServer({
    ...
    verifyClient: (info, cb) => {
        ...
        logger.warn("Connection rejected by origin", {
            origin,
    ...
    if (tokenParam) {
        try {
            ...
            if (secret) {
                ...
                logger.info("JWT verified for user", {
                    userId,
                    platforms: Object.keys(decoded).filter(
                        k => k !== "sub" && k !== "iat" && k !== "exp"
                    ),
                })
            }
...
async function handleYouTubeConnect(ws, connInfo, token, preferredLiveChatId) {
    ...
    relay.on("connected", liveChatId => {
        logger.info("YouTube gRPC stream connected", {
            liveChatId,
    ...
    relay.on("disconnected", reason => {
        logger.info("YouTube gRPC stream disconnected", {
            reason,
            userId: connInfo.userId,
        })
    ...
    relay.on("error", error => {
        logger.error("YouTube gRPC error", {
            error: error.message,
            userId: connInfo.userId,
        })
        if (ws.readyState === 1) {
    ...
    try {
        ...
        logger.error("YouTube relay start failed", {
            error: error.message,
            userId: connInfo.userId,
        })
        if (ws.readyState === 1) {
