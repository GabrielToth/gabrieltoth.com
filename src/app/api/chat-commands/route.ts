...
import { CustomChatCommand, DEFAULT_COMMANDS } from "@/lib/chat/types"
...
export async function GET(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const customCommands: CustomChatCommand[] = (dbCommands || []).map(
            row => ({
                id: row.id,
                trigger: row.trigger,
        ...
        const customTriggers = new Set(
            customCommands.map(c => c.trigger.toLowerCase())
        )
        const mergedCommands = [
            ...customCommands,
            ...DEFAULT_COMMANDS.filter(
                d => !customTriggers.has(d.trigger.toLowerCase())
            ),
        ]
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (body.action === "execute" && body.messageText) {
            const raw = String(body.messageText)
                .trim()
                .split(/\s+/)[0]
                .toLowerCase()
            ...
            try {
                const { data } = await supabaseExec
                    .from("custom_commands")
                    .select("*")
                    .eq("user_id", userId)
                customForExec = (data || []) as Record<string, unknown>[]
            } catch {}
            const customMapped = customForExec.map(
                (row: Record<string, unknown>) => ({
                    trigger: String(row.trigger || ""),
                    responseTemplate: String(
                        row.response_template || row.responseTemplate || ""
                    ),
                    enabled: row.enabled ?? true,
                })
            )
            ...
            const found = all.find(
                (cmd: Record<string, unknown> | CustomChatCommand) => {
                    const trg =
                        typeof cmd.trigger === "string" ? cmd.trigger : ""
            ...
            if (found) {
                ...
                let response = String(
                    f.responseTemplate || f.response_template || ""
                )
                // simple interpolation
                response = response
                    .replace(/{user}/gi, body.username || "user")
                    .replace(/{platform}/gi, body.platform || "")
                // if no template, fallback for discord
                if (!response && raw === "!discord")
                    response =
                        "Join our Discord: https://discord.gg/gabrieltoth"
                return NextResponse.json({
                    success: true,
                    data: {
                        matched: true,
