...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (body.action === "execute" && body.messageText) {
            ...
            let customForExec: any[] = [];
            try {
                const { data } = await supabaseExec.from("custom_commands").select("*").eq("user_id", userId);
                customForExec = data || [];
            } catch {}
            const customMapped = customForExec.map((row: any) => ({
                trigger: row.trigger,
                responseTemplate: row.response_template || row.responseTemplate,
                enabled: row.enabled ?? true,
            }));
            const all = [...customMapped, ...DEFAULT_COMMANDS];
            const found = all.find((cmd: any) => cmd.trigger.toLowerCase() === raw);
            if (found) {
                let response = (found as any).responseTemplate || (found as any).response_template || "";
                ...
                return NextResponse.json({ success: true, data: { matched: true, response, trigger: found.trigger }, matched: true, response });
            }
