...
export function createTokenKillerCLI(pool: DatabasePool): Command {
  ...
  program
    .name('token-killer')
    .description('Token Killer CLI - Token consumption tracking and reporting')
    .version('1.0.0')
  ...
  program
    ...
    .action(async (options) => {
      try {
        ...
        console.error(`Error: Failed to retrieve token statistics: ${message}`)
        logger.error('Failed to retrieve token statistics', error as Error)
        process.exit(1)
      }
    })
