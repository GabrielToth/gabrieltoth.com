...
export function createTokenKillerRouter(pool: DatabasePool): Router {
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const records = await pool.execute<any[]>(query, [
          start.toISOString(),
          end.toISOString(),
        ], { readonly: true })

        // Aggregate data by agent type, request type, and model
        const byAgentType: Record<string, { tokens: number; cost: number; count: number }> = {}
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const breakdownTypeStr = Array.isArray(breakdownType) ? breakdownType[0] : breakdownType
        ...
        const validBreakdownTypes = ['agent-type', 'request-type', 'model', 'strategy']
        ...
        if (breakdownType === 'agent-type' || breakdownType === 'model') {
          query = `
            SELECT
        ...
        const totalTokens = breakdown.reduce((sum, item) => sum + item.totalTokens, 0)
        for (const item of breakdown) {
          item.percentage = totalTokens > 0 ? (item.totalTokens / totalTokens) * 100 : 0
        }
        ...
        const message = error instanceof Error ? error.message : String(error)
        logger.error('Failed to retrieve token breakdown', error as Error)
        res.status(500).json(
          createErrorResponse(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const anomalies = records
          .map((record, index) => {
            const zScore = stdDev > 0 ? (record.dailyTokens - mean) / stdDev : 0
            return {
              index,
              date: record.date,
   ...
   */
  router.get('/api/token-killer/health', async (req: Request, res: Response) => {
    try {
