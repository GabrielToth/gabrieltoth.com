...
export class StorageManager extends EventEmitter {
  ...
  constructor(pool: DatabasePool, config: Partial<ArchivalConfig> = {}) {
    ...
    this.config = {
      ...
      archiveFolder: config.archiveFolder ?? path.join(process.cwd(), '.kiro', 'data', 'archive'),
      ...
      if (stats.totalSize >= STORAGE_THRESHOLDS.ARCHIVAL) {
        ...
        if (this.lastWarningLevel !== 'critical') {
          const warning: StorageWarning = {
            ...
            percentageUsed: (stats.totalSize / STORAGE_THRESHOLDS.ARCHIVAL) * 100,
            timestamp: new Date(),
        ...
        if (this.lastWarningLevel !== 'warning') {
          const warning: StorageWarning = {
            ...
            percentageUsed: (stats.totalSize / STORAGE_THRESHOLDS.WARNING) * 100,
            timestamp: new Date(),
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        connection.get(
          ...
          (err, row: any) => {
            if (err) {
              reject(new Error(`Failed to get active record count: ${err.message}`))
              return
            }
            ...
            connection.get(
              ...
              (err, archivedRow: any) => {
                if (err) {
                  reject(new Error(`Failed to get archived record count: ${err.message}`))
                  return
                }

                const archivedRecordCount = archivedRow?.archivedRecordCount || 0
                const archivedSize = archivedRow?.archivedSize || 0
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        connection.all(
          ...
          async (err, rows: any[]) => {
            if (err) {
              reject(new Error(`Failed to fetch records for archival: ${err.message}`))
              return
            }
            ...
            try {
              ...
              connection.run(
                ...
                async (err) => {
                  if (err) {
                    reject(new Error(`Failed to store archived data: ${err.message}`))
                    return
                  }
                  ...
                  connection.run(
                    ...
                    (err) => {
                      ...
                      const event: ArchivalEvent = {
                        archivedRecordCount: rows.length,
                        originalSize,
                        compressedSize,
                        compressionRatio,
                        timestamp: new Date(),
              ...
              const message = error instanceof Error ? error.message : String(error)
              reject(new Error(`Failed to compress data: ${message}`))
            }
          }
        )
      })
      ...
      return new Promise(async (resolve, reject) => {
        connection.get(
          ...
          async (err, row: any) => {
            ...
            try {
              const decompressedData = await gunzip(row.compressedData)
              const records = JSON.parse(decompressedData.toString('utf8'))
              resolve(records)
            } catch (error) {
              const message = error instanceof Error ? error.message : String(error)
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.all(
          ...
          (err, rows: any[]) => {
            if (err) {
              reject(new Error(`Failed to list archives: ${err.message}`))
              return
            }

            const archives: ArchiveMetadata[] = (rows || []).map((row) => ({
              id: row.id,
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.run(
          ...
          (err) => {
            if (err) {
              reject(new Error(`Failed to delete archive: ${err.message}`))
            } else {
        ...
        const insertNext = () => {
          if (inserted >= records.length) {
            ...
            connection.run(
              ...
              (err) => {
                if (err) {
                  reject(new Error(`Failed to delete archive after restoration: ${err.message}`))
                } else {
          ...
          connection.run(
            ...
            (err) => {
              if (err) {
                reject(new Error(`Failed to restore record: ${err.message}`))
              } else {
                inserted++
                insertNext()
              }
