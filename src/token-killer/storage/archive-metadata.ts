...
export class ArchiveMetadataTracker {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.run(
          ...
          (err) => {
            if (err) {
              reject(new Error(`Failed to track archival metadata: ${err.message}`))
            } else {
              resolve()
            }
          }
   ...
   */
  async getArchiveMetadata(archiveId: string): Promise<ArchiveMetadataEntry | null> {
    try {
      const connection = this.pool.getConnection()
      ...
      return new Promise((resolve, reject) => {
        connection.get(
          ...
          (err, row: any) => {
            if (err) {
              reject(new Error(`Failed to get archive metadata: ${err.message}`))
            } else if (!row) {
              resolve(null)
            } else {
              resolve({
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.all(query, params, (err, rows: any[]) => {
          if (err) {
            reject(new Error(`Failed to query archives: ${err.message}`))
          } else {
   ...
   */
  async getArchivesByDataType(dataType: string): Promise<ArchiveMetadataEntry[]> {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.all(
          ...
          (err, rows: any[]) => {
            if (err) {
              reject(new Error(`Failed to get archives by tags: ${err.message}`))
            } else {
              const filtered = (rows || [])
                .filter((row) => {
                  if (!row.tags) return false
                  const archiveTags = JSON.parse(row.tags)
                  return tags.some((tag) => archiveTags.includes(tag))
                })
                .map((row) => ({
                  archiveId: row.archiveId,
                  recordCount: row.recordCount,
  ...
  async updateArchiveMetadata(
    ...
    try {
      ...
      if (updates.expiresAt !== undefined) {
        ...
        params.push(updates.expiresAt ? updates.expiresAt.toISOString() : null)
      }
      ...
      return new Promise((resolve, reject) => {
        connection.run(
          ...
          (err) => {
            if (err) {
              reject(new Error(`Failed to update archive metadata: ${err.message}`))
            } else {
              resolve()
            }
          }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.run(
          ...
          (err) => {
            if (err) {
              reject(new Error(`Failed to delete archive metadata: ${err.message}`))
            } else {
              resolve()
            }
          }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.get(
          ...
          async (err, row: any) => {
            if (err) {
              reject(new Error(`Failed to get archive stats: ${err.message}`))
              return
            }
            ...
            connection.all(
              ...
              (err, methodRows: any[]) => {
                if (err) {
                  reject(new Error(`Failed to get compression method stats: ${err.message}`))
                  return
                }

                const byMethod = {
                  gzip: { count: 0, totalSize: 0, totalCompressed: 0 },
                  brotli: { count: 0, totalSize: 0, totalCompressed: 0 },
                }

                for (const methodRow of methodRows || []) {
                  if (methodRow.compressionMethod === 'gzip') {
                    byMethod.gzip = {
                      count: methodRow.count,
                      totalSize: methodRow.totalSize || 0,
                      totalCompressed: methodRow.totalCompressed || 0,
                    }
                  } else if (methodRow.compressionMethod === 'brotli') {
                    byMethod.brotli = {
                      count: methodRow.count,
                      totalSize: methodRow.totalSize || 0,
                      totalCompressed: methodRow.totalCompressed || 0,
                ...
                resolve({
                  ...
                  totalRecordsArchived: row.totalRecordsArchived || 0,
                  totalOriginalSize: row.totalOriginalSize || 0,
                  totalCompressedSize: row.totalCompressedSize || 0,
                  averageCompressionRatio: row.averageCompressionRatio || 0,
                  oldestArchive: row.oldestArchive ? new Date(row.oldestArchive) : undefined,
                  newestArchive: row.newestArchive ? new Date(row.newestArchive) : undefined,
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.run(
          ...
          function (err) {
            if (err) {
              reject(new Error(`Failed to cleanup expired archives: ${err.message}`))
            } else {
...
export { ArchiveMetadataEntry, ArchiveMetadataQueryOptions, ArchiveMetadataStats }
