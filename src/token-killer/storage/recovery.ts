...
export class StorageRecoveryManager extends EventEmitter {
  ...
  constructor(pool: DatabasePool, backupDir?: string) {
    super()
    this.pool = pool
    this.backupDir = backupDir || path.join(process.cwd(), '.kiro', 'data', 'backups')
  }
  ...
  async checkIntegrity(): Promise<IntegrityCheckResult> {
    ...
    try {
      ...
      const fileCheckPassed = fs.existsSync(dbPath) && fs.statSync(dbPath).size > 0
      checks.push({
        name: 'Database file exists and readable',
        passed: fileCheckPassed,
        message: fileCheckPassed ? 'Database file is valid' : 'Database file not found or empty',
      ...
      checks.push({
        ...
        message: health.healthy ? 'Database is responsive' : `Database error: ${health.error}`,
      ...
      checks.push({
        ...
        message: schemaCheck.valid ? 'All tables and indexes present' : 'Schema issues detected',
        details: schemaCheck,
      })
      ...
      checks.push({
        ...
        message: fkCheck.valid ? 'All foreign keys valid' : 'Foreign key violations detected',
        details: { violationCount: fkCheck.violations.length },
      })

      if (!fkCheck.valid) {
        warnings.push(`Found ${fkCheck.violations.length} foreign key violations`)
      }
      ...
      checks.push({
        ...
        message: dataCheck.valid ? 'Data is consistent' : 'Data inconsistencies detected',
        details: dataCheck,
      })
      ...
      checks.push({
        ...
        message: constraintCheck.valid ? 'No constraint violations' : 'Constraint violations found',
        details: { violationCount: constraintCheck.violations.length },
      })

      if (!constraintCheck.valid) {
        warnings.push(`Found ${constraintCheck.violations.length} constraint violations`)
      }
      ...
      checks.push({
        ...
        message: archiveCheck.valid ? 'All archives valid' : 'Archive issues detected',
        ...
        connection.get(
          ...
          (err, row) => {
            if (err) {
              ...
              errors.push(`Required table '${tableName}' not found`)
            }
            checked++
            checkNextTable()
          }
  ...
  private async checkForeignKeyConstraints(): Promise<{
    ...
    return new Promise((resolve) => {
      connection.all(`PRAGMA foreign_key_list(token_records)`, (err, rows: any[]) => {
        if (err) {
        ...
        connection.all(
          ...
          (err, orphanedRecords: any[]) => {
            if (!err && orphanedRecords && orphanedRecords.length > 0) {
      ...
      connection.all(
        ...
        (err, row: any) => {
          if (!err && row && row.count > 0) {
            issues.push(`Found ${row.count} token records with negative values`)
          }
          ...
          connection.all(
            ...
            (err, row: any) => {
              ...
              connection.all(
                ...
                (err, row: any) => {
                  if (!err && row && row.count > 0) {
                    issues.push(`Found ${row.count} budget usage records exceeding limits`)
                  }
        ...
        async (err, rows: any[]) => {
          ...
          const checkNextArchive = async () => {
            ...
            try {
              ...
              const message = error instanceof Error ? error.message : String(error)
              errors.push(`Archive ${archive.id} is corrupted: ${message}`)
              checked++
              checkNextArchive()
            }
          }
  ...
  async recover(): Promise<RecoveryResult> {
    ...
    try {
      ...
      if (!fkCheck.valid && fkCheck.violations.length > 0) {
        ...
        for (const violation of fkCheck.violations) {
          try {
            ...
            const message = error instanceof Error ? error.message : String(error)
            warnings.push(`Failed to fix foreign key violation: ${message}`)
          }
        }
      ...
      const constraintCheck = await this.checkConstraintViolations()
      if (!constraintCheck.valid && constraintCheck.violations.length > 0) {
        const connection = this.pool.getConnection()

        for (const violation of constraintCheck.violations) {
          try {
            if (violation.table === 'token_records' && violation.constraint === 'agentType CHECK') {
              await new Promise<void>((resolve, reject) => {
                connection.run(
            ...
            const message = error instanceof Error ? error.message : String(error)
            warnings.push(`Failed to fix constraint violation: ${message}`)
          }
        }
  ...
  async createBackup(description?: string, tags?: string[]): Promise<BackupMetadata> {
    try {
      ...
      // Get database stats
      const connection = this.pool.getConnection()
      const stats = await new Promise<{ recordCount: number; archivedRecordCount: number }>(
        (resolve) => {
          connection.get(
            ...
            (err, row: any) => {
              ...
              connection.get(
                ...
                (err, archivedRow: any) => {
                  const archivedRecordCount = archivedRow?.archivedRecordCount || 0
