...
export class ArchiveRestorationService extends EventEmitter {
   ...
   */
  async restoreArchive(options: RestorationOptions): Promise<RestorationResult> {
    if (this.isRestoring) {
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        const metadata = await this.metadataTracker.getArchiveMetadata(options.archiveId)
        ...
        connection.get(
          ...
          async (err, row: any) => {
            if (err) {
              this.isRestoring = false
              reject(new Error(`Failed to fetch archive: ${err.message}`))
              return
            }

            if (!row) {
              this.isRestoring = false
              reject(new Error(`Archive data not found: ${options.archiveId}`))
              return
            }
            ...
            try {
              ...
              const skippedRecords = totalRecords - recordsToRestore.length
              ...
              const restoreNextBatch = () => {
                    ...
                    connection.run(
                      ...
                      async (err) => {
                        if (err) {
                          this.isRestoring = false
                          reject(
                            new Error(`Failed to delete archive after restoration: ${err.message}`)
                          )
                        } else {
                          // Delete metadata
                          await this.metadataTracker.deleteArchiveMetadata(options.archiveId)
                ...
                // Get next batch
                const batch = recordsToRestore.slice(restoredCount, restoredCount + batchSize)
                ...
                const insertNextRecord = () => {
                  if (inserted >= batch.length) {
                    ...
                    // Update progress
                    const elapsedTime = Date.now() - startRestoreTime
                    const recordsPerMs = restoredCount / elapsedTime
                    const remainingRecords = recordsToRestore.length - restoredCount
                    ...
                    this.restorationProgress = {
                      ...
                      skippedRecords,
                      percentageComplete: (restoredCount / recordsToRestore.length) * 100,
                      estimatedTimeRemaining,
                    }

                    this.emit('restoration_progress', this.restorationProgress)
              ...
              reject(new Error(`Failed to restore archive: ${message}`))
            }
          }
        )
      })
   ...
   */
  async restoreAllArchives(deleteAfterRestore: boolean = false): Promise<RestorationResult[]> {
    try {
