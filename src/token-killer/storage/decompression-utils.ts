...
export class TransparentDecompressionUtil {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        connection.all(query, params, (err, rows: any[]) => {
          if (err) {
            reject(new Error(`Failed to query active data: ${err.message}`))
          } else {
            const records = rows || []
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        connection.get(
          ...
          async (err, row: any) => {
            if (err) {
              reject(new Error(`Failed to fetch archived data: ${err.message}`))
              return
            }
            ...
            try {
              ...
              const message = error instanceof Error ? error.message : String(error)
              reject(new Error(`Failed to decompress archived data: ${message}`))
            }
          }
        )
      })
  ...
  async queryAllArchives(
    ...
    try {
      ...
      for (const archive of archives) {
        const result = await this.queryArchived(archive.archiveId, predicate)
        results.push(result)
      }
   ...
   */
  async unifiedQuery(options: UnifiedQueryOptions = {}): Promise<DecompressedQueryResult[]> {
    const results: DecompressedQueryResult[] = []
      ...
      if (options.includeArchived) {
        if (options.archiveIds && options.archiveIds.length > 0) {
          ...
          for (const archiveId of options.archiveIds) {
            const archivedResult = await this.queryArchived(archiveId, options.predicate)
            results.push(archivedResult)
          }
        } else {
          // Query all archives
          const archivedResults = await this.queryAllArchives(options.predicate)
   ...
   */
  async getRecordsByAgentType(agentType: string): Promise<DecompressedQueryResult[]> {
   ...
   */
  async getRecordsByTaskId(taskId: string): Promise<DecompressedQueryResult[]> {
   ...
   */
  async getRecordsByRequestId(requestId: string): Promise<DecompressedQueryResult[]> {
      ...
      return new Promise((resolve, reject) => {
        connection.get(
          ...
          async (err, activeRow: any) => {
            if (err) {
              reject(new Error(`Failed to count active records: ${err.message}`))
              return
            }
            ...
            connection.get(
              ...
              (err, archivedRow: any) => {
                if (err) {
                  reject(new Error(`Failed to count archived records: ${err.message}`))
                } else {
                  const activeCount = activeRow?.activeCount || 0
                  const archivedCount = archivedRow?.archivedCount || 0
                  resolve({
                    activeCount,
                    archivedCount,
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        connection.get(
          ...
          async (err, activeRow: any) => {
            if (err) {
              reject(new Error(`Failed to get active stats: ${err.message}`))
              return
            }
            ...
            connection.get(
              ...
              (err, archivedRow: any) => {
                if (err) {
                  reject(new Error(`Failed to get archived stats: ${err.message}`))
                } else {
                  const activeRecords = activeRow?.recordCount || 0
                  const activeTokens = activeRow?.totalTokens || 0
                  const activeCost = activeRow?.totalCost || 0

                  const archivedRecords = archivedRow?.recordCount || 0
                  const archivedTokens = archivedRow?.totalTokens || 0
                  const archivedCost = archivedRow?.totalCost || 0

                  const totalRecords = activeRecords + archivedRecords
                  const totalTokens = activeTokens + archivedTokens
                  ...
                  resolve({
                    ...
                    averageTokensPerRecord: totalRecords > 0 ? totalTokens / totalRecords : 0,
