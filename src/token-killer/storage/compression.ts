...
export class CompressionService {
    ...
    async decompress(
        ...
        try {
            ...
            if (method === "brotli") {
                decompressedBuffer = (await brotliDecompress(
                    compressedData
                )) as Buffer
            } else {
    ...
    estimateCompressionRatio(
        ...
        try {
            ...
            if (method === "brotli") {
                ...
                return (
                    Math.min(originalSize * 0.4, originalSize * 0.5) /
                    originalSize
                )
            } else {
                // Gzip typically achieves 50-70% compression on JSON
                return (
                    Math.min(originalSize * 0.5, originalSize * 0.6) /
                    originalSize
                )
            }
        } catch (error) {
    ...
    async compareCompressionMethods(data: any[]): Promise<{
        ...
        try {
            ...
            const recommendation =
                brotliResult.compressionRatio > gzipResult.compressionRatio
                    ? "brotli"
                    : "gzip"
