...
export class ArchivalIntegrationService extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to initialize archival integration: ${message}`)
    }
  }
   ...
   * Query archived data
   */
  async queryArchived(archiveId: string, predicate?: (record: any) => boolean): Promise<any[]> {
    const result = await this.decompressionUtil.queryArchived(archiveId, predicate)
    return result.records
  }
   ...
   */
  async getRecordsByDateRange(startDate: Date, endDate: Date): Promise<any[]> {
    const results = await this.decompressionUtil.getRecordsByDateRange(startDate, endDate)
  ...
  async getRecordsByAgentType(agentType: string): Promise<any[]> {
    const results = await this.decompressionUtil.getRecordsByAgentType(agentType)
    return results.flatMap((r) => r.records)
  }
  ...
  async getRecordsByRequestId(requestId: string): Promise<any[]> {
    const results = await this.decompressionUtil.getRecordsByRequestId(requestId)
    return results.flatMap((r) => r.records)
  }
  ...
  async search(searchTerm: string, searchFields?: string[]): Promise<any[]> {
    const results = await this.decompressionUtil.search(searchTerm, searchFields)
    return results.flatMap((r) => r.records)
  }
   ...
   * Restore archive
   */
  async restoreArchive(archiveId: string, deleteAfterRestore: boolean = false): Promise<any> {
  ...
  async restoreMultipleArchives(
    ...
    return this.restorationService.restoreMultipleArchives(archiveIds, deleteAfterRestore)
  }

  /**
   * Restore all archives
   */
  async restoreAllArchives(deleteAfterRestore: boolean = false): Promise<any[]> {
