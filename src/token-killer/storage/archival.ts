...
export class ArchivalService extends EventEmitter {
    ...
    startScheduling(): void {
        if (
            !this.config.autoArchivalEnabled ||
            this.config.archivalSchedule === "manual"
        ) {
            return
        }
                ...
                connection.get(
                    ...
                    async (err, countRow: any) => {
                        if (err) {
                            ...
                            reject(
                                new Error(
                                    `Failed to count records for archival: ${err.message}`
                                )
                            )
                            return
                        }
                        ...
                        try {
                            ...
                            reject(
                                new Error(`Failed to archive data: ${message}`)
                            )
                        }
                    }
                )
            ...
            connection.all(
                ...
                (err, rows: any[]) => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to fetch records batch: ${err.message}`
                            )
                        )
                    } else {
                        resolve(rows || [])
    ...
    private storeArchive(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                ...
                err => {
                    if (err) {
                        reject(
                            new Error(`Failed to store archive: ${err.message}`)
                        )
                    } else {
                        resolve()
                    }
