...
describe('Tokenizer - Property-Based Tests', () => {
  describe('Property 1: Token Count Consistency', () => {
    ...
    it('should return consistent token counts for same input', () => {
      fc.assert(
        fc.property(fc.string(), (text) => {
          const count1 = TokenizerFactory.countInputTokens(text, 'claude-haiku-4.5')
          const count2 = TokenizerFactory.countInputTokens(text, 'claude-haiku-4.5')
          const count3 = TokenizerFactory.countInputTokens(text, 'claude-haiku-4.5')
    ...
    it('should return 0 tokens for empty string', () => {
      const count = TokenizerFactory.countInputTokens('', 'claude-haiku-4.5')
      expect(count).toBe(0)
    })
    ...
    it('should scale token count with text length', () => {
      fc.assert(
        fc.property(fc.string(), fc.string(), (text1, text2) => {
          const count1 = TokenizerFactory.countInputTokens(text1, 'claude-haiku-4.5')
          const count2 = TokenizerFactory.countInputTokens(text1 + text2, 'claude-haiku-4.5')
      ...
      fc.assert(
        fc.property(fc.string({ minLength: 1 }), (text) => {
          ...
          return models.every((model) => {
            const count = TokenizerFactory.countInputTokens(text, model)
            return count > 0
          })
        })
      )
    ...
    it('should produce similar token counts across models', () => {
      fc.assert(
        fc.property(fc.string({ minLength: 1 }), (text) => {
          ...
          if (claudeCount <= 0 || geminiCount <= 0 || cursorCount <= 0) {
            return false
          }
    ...
    it('should verify token counts consistently', () => {
      fc.assert(
        fc.property(fc.string(), (text) => {
          const count = TokenizerFactory.countInputTokens(text, 'claude-haiku-4.5')
          const verified1 = TokenizerFactory.verifyTokenCount(text, 'claude-haiku-4.5', count)
          const verified2 = TokenizerFactory.verifyTokenCount(text, 'claude-haiku-4.5', count)
    ...
    it('should maintain monotonic token count', () => {
      fc.assert(
        fc.property(fc.string(), fc.string(), (text1, text2) => {
          const count1 = TokenizerFactory.countInputTokens(text1, 'claude-haiku-4.5')
          const count2 = TokenizerFactory.countInputTokens(text1 + text2, 'claude-haiku-4.5')
          const count3 = TokenizerFactory.countInputTokens(text1 + text2 + text2, 'claude-haiku-4.5')
    ...
    it('should count whitespace characters', () => {
      fc.assert(
        fc.property(fc.string({ minLength: 1 }).filter((s) => /^\s+$/.test(s)), (text) => {
          const count = TokenizerFactory.countInputTokens(text, 'claude-haiku-4.5')
          return count > 0
        })
      )
    ...
    it('should scale linearly with repeated text', () => {
      fc.assert(
        fc.property(fc.string({ minLength: 1 }), fc.integer({ min: 2, max: 5 }), (text, n) => {
          const singleCount = TokenizerFactory.countInputTokens(text, 'claude-haiku-4.5')
          ...
          return Math.abs(ratio - expectedRatio) / expectedRatio <= 0.2
        })
      )
    })
  })
