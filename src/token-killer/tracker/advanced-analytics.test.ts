...
vi.mock('./analytics', () => {
  class MockAnalyticsEngine {
    computeStatistics = vi.fn().mockResolvedValue({
      ...
      max: 200,
      range: 100,
      percentiles: { p25: 125, p50: 150, p75: 175, p90: 190, p95: 195, p99: 199 },
    })
    ...
    detectPatterns = vi.fn().mockResolvedValue({
      ...
      byRequestType: {},
      byModel: { 'claude-haiku-4.5': { tokens: 450, count: 3, avgTokens: 150 } },
    })
...
describe('AdvancedAnalyticsService', () => {
  ...
  describe('generateVisualizationAnalytics', () => {
    it('should generate visualization-ready analytics data', async () => {
      const mockConnection = {
        all: vi.fn((query, params, callback) => {
          if (query.includes('SELECT totalTokens')) {
            ...
            callback(null, [
              { timestamp: '2024-01-01T09:00:00Z', totalTokens: 100, totalCost: 0.01 },
              { timestamp: '2024-01-01T10:00:00Z', totalTokens: 200, totalCost: 0.02 },
              { timestamp: '2024-01-01T11:00:00Z', totalTokens: 150, totalCost: 0.015 },
            ])
          } else {
            callback(null, [])
          }
      ...
      const analytics = await service.generateVisualizationAnalytics(startDate, endDate)
    ...
    it('should cache results and return cached data on subsequent calls', async () => {
      ...
      const result1 = await service.generateVisualizationAnalytics(startDate, endDate)
      expect(result1).toBeDefined()

      // Second call - should use cache
      const result2 = await service.generateVisualizationAnalytics(startDate, endDate)
      expect(result2).toBeDefined()

      // Results should be identical
      expect(result1.summary.totalTokens).toBe(result2.summary.totalTokens)
    ...
    it('should bypass cache when useCache is false', async () => {
      ...
      await service.generateVisualizationAnalytics(startDate, endDate, true)

      // Second call without cache
      const result = await service.generateVisualizationAnalytics(startDate, endDate, false)
    ...
    it('should calculate anomaly percentage correctly', async () => {
      const mockConnection = {
        all: vi.fn((query, params, callback) => {
          if (query.includes('SELECT totalTokens')) {
            ...
            callback(null, [
              { timestamp: '2024-01-01T09:00:00Z', totalTokens: 100, totalCost: 0.01 },
              { timestamp: '2024-01-01T10:00:00Z', totalTokens: 100, totalCost: 0.01 },
              { timestamp: '2024-01-01T11:00:00Z', totalTokens: 100, totalCost: 0.01 },
              { timestamp: '2024-01-01T12:00:00Z', totalTokens: 1000, totalCost: 0.1 },
            ])
          } else {
            callback(null, [])
          }
      ...
      expect(analytics.summary.anomalyPercentage).toBeGreaterThanOrEqual(0)
  ...
  describe('getCachedStatistics', () => {
    it('should cache statistics and return cached result', async () => {
      ...
      // First call
      const result1 = await service.getCachedStatistics(startDate, endDate)
      ...
      // Second call should use cache
      const result2 = await service.getCachedStatistics(startDate, endDate)
    ...
    it('should force refresh when forceRefresh is true', async () => {
      ...
      // Force refresh
      const result = await service.getCachedStatistics(startDate, endDate, true)
  ...
  describe('getCachedAnomalies', () => {
    it('should cache anomalies with threshold', async () => {
      ...
      const result = await service.getCachedAnomalies(startDate, endDate, 2)
      expect(result.data).toBeDefined()
      expect(result.isExpired).toBe(false)
    })
  })
  ...
  describe('getCachedForecast', () => {
    it('should cache forecast with forecast days', async () => {
      ...
      const result = await service.getCachedForecast(startDate, endDate, 7)
      expect(result.data).toBeDefined()
      expect(result.isExpired).toBe(false)
    })
  })
  ...
  describe('Cache Management', () => {
    ...
    it('should set cache expiration time', () => {
      ...
      expect(() => service.setCacheExpiration(0)).toThrow('Cache expiration must be positive')
      expect(() => service.setCacheExpiration(-1)).toThrow('Cache expiration must be positive')
    })
  })
