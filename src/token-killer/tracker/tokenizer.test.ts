...
describe("Tokenizer Module", () => {
    ...
    describe("ClaudeTokenizer", () => {
        ...
        it("should verify token count within acceptable range", () => {
            ...
            expect(
                tokenizer.verifyTokenCount(
                    text,
                    Math.ceil(calculatedCount * 1.05)
                )
            ).toBe(true)
            ...
            if (calculatedCount > 5) {
                expect(
                    tokenizer.verifyTokenCount(
                        text,
                        Math.ceil(calculatedCount * 1.15)
                    )
                ).toBe(false)
            }
        })
    ...
    describe("TokenizerFactory", () => {
        ...
        it("should count output tokens for a model", () => {
            ...
            const count = TokenizerFactory.countOutputTokens(
                text,
                "claude-haiku-4.5"
            )
            expect(count).toBeGreaterThan(0)
        ...
        it("should verify token count for a model", () => {
            ...
            const verified = TokenizerFactory.verifyTokenCount(
                text,
                "claude-haiku-4.5",
                count
            )
    ...
    describe("Consistency Properties", () => {
        ...
        it("should scale linearly with text length", () => {
            ...
            const baseCount = TokenizerFactory.countInputTokens(
                baseText,
                "claude-haiku-4.5"
            )

            const doubledText = baseText + baseText
            const doubledCount = TokenizerFactory.countInputTokens(
                doubledText,
                "claude-haiku-4.5"
            )
    ...
    describe("Fallback Mechanism", () => {
        ...
        it("should never throw on invalid input", () => {
            const invalidInputs = [null, undefined, NaN, Infinity]
