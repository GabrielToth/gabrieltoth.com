    ...
    fc.assert(
      fc.property(
        ...
        (usdCost, exchangeRate) => {
          const reporter2 = new TokenReporter(new PricingManager(exchangeRate), exchangeRate)
          ...
          return Math.abs(expectedBRL - (usdCost * exchangeRate)) < 0.000001
        }
      ),
      { numRuns: 100 }
    )
  ...
  it('should maintain invariant: agent aggregation sums correctly', () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
            agentType: fc.constantFrom('kiro', 'antigravity', 'cursor', 'gabrieltoth'),
        ...
        (records) => {
          ...
          for (const record of records) {
            ...
            agentTotals[record.agentType] = (agentTotals[record.agentType] || 0) + record.tokens
          }
    ...
    fc.assert(
      fc.property(
        ...
        (inputCost, outputCost) => {
          ...
          return Math.abs(totalCost - (inputCost + outputCost)) < 0.000001
        }
      ),
      { numRuns: 100 }
    )
      ...
      fc.property(
        ...
        (data) => {
          ...
          return (
            ...
            Math.abs(parsed.totalCostUSD - data.totalCostUSD) < 0.000001
          )
        }
      ),
      { numRuns: 100 }
