...
describe('PricingManager', () => {
  ...
  describe('Exchange Rate', () => {
    ...
    it('should reject negative exchange rate', () => {
      expect(() => manager.setExchangeRate(-1)).toThrow('Exchange rate must be positive')
    })

    it('should reject zero exchange rate', () => {
      expect(() => manager.setExchangeRate(0)).toThrow('Exchange rate must be positive')
    })
  })
  ...
  describe('Currency Conversion', () => {
    ...
    it('should reject negative amounts in USD to BRL', () => {
      expect(() => manager.convertUSDToBRL(-100)).toThrow('Amount must be non-negative')
    })

    it('should reject negative amounts in BRL to USD', () => {
      expect(() => manager.convertBRLToUSD(-500)).toThrow('Amount must be non-negative')
    })
  ...
  describe('Pricing Overrides', () => {
    ...
    it('should reject negative prices in override', () => {
      expect(() => manager.setPricingOverride('claude-haiku-4.5', -1.0, 5.0)).toThrow(
  ...
  describe('Cost Calculation', () => {
    it('should calculate cost for Claude model', async () => {
      const cost = await manager.calculateCost('claude-haiku-4.5', 1000, 500)
    ...
    it('should calculate cost for Gemini model', async () => {
      const cost = await manager.calculateCost('gemini-flash-3.1', 1000, 500)
    ...
    it('should calculate cost for Cursor model', async () => {
      const cost = await manager.calculateCost('cursor-composer-2.0', 1000, 500)
    ...
    it('should reject negative token counts', async () => {
      await expect(manager.calculateCost('claude-haiku-4.5', -1000, 500)).rejects.toThrow(
  ...
  describe('Pricing Verification', () => {
    it('should indicate when pricing needs verification after interval', () => {
      ...
      ;(freshManager as any).lastPricingPrompt = new Date(Date.now() - 8 * 24 * 60 * 60 * 1000)
