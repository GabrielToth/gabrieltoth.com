        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (inputPrice, outputPrice, inputTokens, outputTokens) => {
                    ...
                    manager2.setPricingOverride(
                        "claude-haiku-4.5",
                        inputPrice,
                        outputPrice
                    )

                    const cost = await manager2.calculateCost(
                        "claude-haiku-4.5",
                        inputTokens,
                        outputTokens
                    )
    ...
    it("should maintain invariant: model-specific pricing is applied", async () => {
        await fc.assert(
            fc.asyncProperty(
                ...
                fc.integer({ min: 1000, max: 100000 }),
                async (inputTokens, outputTokens) => {
                    const claudeCost = await manager.calculateCost(
                        "claude-haiku-4.5",
                        inputTokens,
                        outputTokens
                    )
                    const geminiCost = await manager.calculateCost(
                        "gemini-flash-3.1",
                        inputTokens,
                        outputTokens
                    )
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (baseTokens, additionalTokens) => {
                    const cost1 = await manager.calculateCost(
                        "claude-haiku-4.5",
                        baseTokens,
                        0
                    )
                    const cost2 = await manager.calculateCost(
                        "claude-haiku-4.5",
                        baseTokens + additionalTokens,
                        0
                    )
