...
export interface ComprehensiveReport {
    ...
    byAgent: Record<
        AgentType,
        { tokens: number; cost: number; count: number; percentage: number }
    >
    byModel: Record<
        SupportedModel,
        { tokens: number; cost: number; count: number; percentage: number }
...
export class ComprehensiveReporter {
    ...
    async generateComprehensiveReport(
        ...
        try {
            ...
            report.byAgent = await this.aggregateByAgent(
                startDate,
                endDate,
            ...
            report.byModel = await this.aggregateByModel(
                startDate,
                endDate,
                summary.totalCostUSD
            )
            ...
            report.byRequestType = await this.aggregateByRequestType(
                startDate,
                endDate,
                summary.totalCostUSD
            )
            ...
            report.statistics = await this.analyticsEngine.computeStatistics(
                startDate,
                endDate
            )
            ...
            report.patterns = await this.analyticsEngine.detectPatterns(
                startDate,
                endDate
            )
            ...
            try {
                report.forecast =
                    await this.analyticsEngine.forecastConsumption(
                        startDate,
                        endDate,
                        7
                    )
            ...
            throw new Error(
                `Failed to generate comprehensive report: ${error instanceof Error ? error.message : String(error)}`
            )
        }
    }
    ...
    async exportAsCSV(report: ComprehensiveReport): Promise<string> {
        ...
        lines.push(
            `Average Tokens/Request,${report.summary.averageTokensPerRequest.toFixed(2)}`
        )
        lines.push(
            `Average Cost/Request,$${report.summary.averageCostPerRequest.toFixed(6)}`
        )
        lines.push("")
        ...
        if (report.forecast) {
            ...
            lines.push("Metric,Value")
            lines.push(
                `Forecasted Tokens,${report.forecast.forecastedTokens.toFixed(2)}`
            )
            lines.push(
                `95% CI Lower,${report.forecast.confidenceInterval95.lower.toFixed(2)}`
            )
            lines.push(
                `95% CI Upper,${report.forecast.confidenceInterval95.upper.toFixed(2)}`
            )
            lines.push(
                `99% CI Lower,${report.forecast.confidenceInterval99.lower.toFixed(2)}`
            )
            lines.push(
                `99% CI Upper,${report.forecast.confidenceInterval99.upper.toFixed(2)}`
            )
                ...
                .all(
                    ...
                    (err, rows: any[]) => {
                        ...
                        for (const row of rows || []) {
                            ...
                            result[requestType].percentage =
                                totalCost > 0
                                    ? (result[requestType].cost / totalCost) *
                                      100
                                    : 0
                        }
            ...
            this.pool
                ...
                .all(
                    ...
                    (err, rows: any[]) => {
                        ...
                        const weeklyTrend = Object.entries(weeklyMap).map(
                            ([week, data]) => ({
                                week,
                                tokens: data.tokens,
                                cost: data.cost,
