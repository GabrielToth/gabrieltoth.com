...
export interface PatternDetectionResult {
    byTimeOfDay: Record<
        string,
        { tokens: number; count: number; avgTokens: number }
    >
    byDayOfWeek: Record<
        string,
        { tokens: number; count: number; avgTokens: number }
...
export class AnalyticsEngine {
                ...
                .all(
                    ...
                    (err, rows: any[]) => {
                        ...
                        if (!rows || rows.length === 0) {
                            reject(
                                new Error("No data available for statistics")
                            )
                            return
                        }
                ...
                .all(
                    ...
                    (err, rows: any[]) => {
                        ...
                        if (!rows || rows.length === 0) {
                            reject(
                                new Error(
                                    "No data available for pattern detection"
                                )
                            )
                            return
                        }
                        ...
                        for (const row of rows) {
                            ...
                            if (!result.byTimeOfDay[timeOfDay]) {
                                result.byTimeOfDay[timeOfDay] = {
                                    tokens: 0,
                                    count: 0,
                                    avgTokens: 0,
                                }
                            }
                            result.byTimeOfDay[timeOfDay].tokens +=
                                row.totalTokens
                            result.byTimeOfDay[timeOfDay].count += 1

                            // By day of week
                            if (!result.byDayOfWeek[dayOfWeek]) {
                                result.byDayOfWeek[dayOfWeek] = {
                                    tokens: 0,
                                    count: 0,
                                    avgTokens: 0,
                                }
                            }
                            result.byDayOfWeek[dayOfWeek].tokens +=
                                row.totalTokens
                            result.byDayOfWeek[dayOfWeek].count += 1

                            // By agent type
                            if (!result.byAgentType[row.agentType]) {
                                result.byAgentType[row.agentType] = {
                                    tokens: 0,
                                    count: 0,
                                    avgTokens: 0,
                                }
                            }
                            result.byAgentType[row.agentType].tokens +=
                                row.totalTokens
                            result.byAgentType[row.agentType].count += 1

                            // By model
                            if (!result.byModel[row.model]) {
                                result.byModel[row.model] = {
                                    tokens: 0,
                                    count: 0,
                                    avgTokens: 0,
                                }
                            ...
                            if (row.metadata) {
                                try {
                                    ...
                                    if (!result.byRequestType[requestType]) {
                                        result.byRequestType[requestType] = {
                                            tokens: 0,
                                            count: 0,
                                            avgTokens: 0,
                                        }
                                    }
                                    result.byRequestType[requestType].tokens +=
                                        row.totalTokens
                        ...
                        for (const key in result.byTimeOfDay) {
                            result.byTimeOfDay[key].avgTokens =
                                result.byTimeOfDay[key].tokens /
                                result.byTimeOfDay[key].count
                        }
                        for (const key in result.byDayOfWeek) {
                            result.byDayOfWeek[key].avgTokens =
                                result.byDayOfWeek[key].tokens /
                                result.byDayOfWeek[key].count
                        }
                        for (const key in result.byAgentType) {
                            result.byAgentType[key].avgTokens =
                                result.byAgentType[key].tokens /
                                result.byAgentType[key].count
                        }
                        for (const key in result.byRequestType) {
                            result.byRequestType[key].avgTokens =
                                result.byRequestType[key].tokens /
                                result.byRequestType[key].count
                        }
                        for (const key in result.byModel) {
                            result.byModel[key].avgTokens =
                                result.byModel[key].tokens /
                                result.byModel[key].count
                        }
            ...
            this.pool
                ...
                .all(
                    ...
                    (err, rows: any[]) => {
                        ...
                        for (const row of rows) {
                            const zScore =
                                (row.totalTokens - stats.mean) / stats.stdDev
    ...
    private calculateResiduals(actual: number[], forecast: number[]): number[] {
        return actual.map(
            (a, i) => a - (forecast[i] || forecast[forecast.length - 1])
        )
    }
}
