...
export class TokenRecorder {
  ...
  async recordToken(request: RecordTokenRequest): Promise<TokenRecord> {
    ...
    if (this.batchBuffer.length >= this.batchSize) {
      ...
      this.flushTimer = setTimeout(() => this.flushBatch(), this.batchFlushInterval)
    }
  ...
  private updateRunningTotal(taskId: string, record: TokenRecord): void {
    ...
    if (existing) {
      ...
      existing.currentCostBRL = existing.currentCostUSD * this.exchangeRate
      ...
      this.runningTotals.set(taskId, {
        ...
        currentCostBRL: (record.inputCost + record.outputCost) * this.exchangeRate,
        requestCount: 1,
  ...
  async aggregateTask(taskId: string): Promise<TaskAggregation> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pool.getConnection().all(query, [taskId], (err, rows: any[]) => {
        if (err) {
          reject(err)
          return
        }
