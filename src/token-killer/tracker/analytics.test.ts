...
describe('AnalyticsEngine', () => {
  ...
  describe('computeStatistics', () => {
    ...
    it('should reject when no data available', async () => {
      ...
      await expect(engine.computeStatistics(startDate, endDate)).rejects.toThrow(
    ...
    it('should handle database errors', async () => {
      ...
      await expect(engine.computeStatistics(startDate, endDate)).rejects.toThrow('Database error')
    })
  })
  ...
  describe('detectAnomalies', () => {
    it('should detect anomalies using z-score', async () => {
      const mockData = [
        { totalTokens: 100, timestamp: '2024-01-01T09:00:00Z', id: '1' },
        { totalTokens: 110, timestamp: '2024-01-01T10:00:00Z', id: '2' },
        { totalTokens: 105, timestamp: '2024-01-01T11:00:00Z', id: '3' },
        { totalTokens: 1000, timestamp: '2024-01-01T12:00:00Z', id: '4' }, // Anomaly
      ...
      const highValueAnomaly = result.anomalies.find((a) => a.totalTokens === 1000)
    ...
    it('should calculate correct deviation percentage', async () => {
      const mockData = [
        { totalTokens: 100, timestamp: '2024-01-01T09:00:00Z', id: '1' },
        { totalTokens: 100, timestamp: '2024-01-01T10:00:00Z', id: '2' },
        { totalTokens: 100, timestamp: '2024-01-01T11:00:00Z', id: '3' },
        { totalTokens: 200, timestamp: '2024-01-01T12:00:00Z', id: '4' }, // 100% above mean
      ]
  ...
  describe('forecastConsumption', () => {
    it('should forecast with sufficient data', async () => {
      ...
      const forecast = await engine.forecastConsumption(startDate, endDate, 7)
      ...
      expect(forecast.confidenceInterval95.upper).toBeGreaterThan(forecast.confidenceInterval95.lower)
      expect(forecast.confidenceInterval99.upper).toBeGreaterThan(forecast.confidenceInterval95.upper)
    ...
    it('should reject with insufficient data', async () => {
      ...
      await expect(engine.forecastConsumption(startDate, endDate, 7)).rejects.toThrow(
    ...
    it('should handle database errors during forecasting', async () => {
      ...
      await expect(engine.forecastConsumption(startDate, endDate, 7)).rejects.toThrow('Database error')
    })
  })
})
