...
describe('ComprehensiveReporter', () => {
  ...
  describe('generateComprehensiveReport', () => {
    it('should generate report with all sections', async () => {
      const mockConnection = {
        ...
        all: vi.fn((query, params, callback) => {
          ...
          if (query.includes('SELECT totalTokens')) {
            ...
            callback(null, [
              { agentType: 'kiro', tokens: 3000, cost: 0.03, count: 6 },
              { agentType: 'cursor', tokens: 2000, cost: 0.02, count: 4 },
            ])
          } else if (query.includes('model')) {
            callback(null, [
              { model: 'claude-haiku-4.5', tokens: 5000, cost: 0.05, count: 10 },
            ])
          } else if (query.includes('metadata')) {
            callback(null, [
              { metadata: JSON.stringify({ requestType: 'spec-creation' }), tokens: 5000, cost: 0.05, count: 10 },
            ])
      ...
      const report = await reporter.generateComprehensiveReport(startDate, endDate)
    ...
    it('should call progress callback with correct stages', async () => {
      ...
      try {
        await reporter.generateComprehensiveReport(startDate, endDate, progressCallback)
      } catch {
      ...
      const mockConnection = {
        ...
        all: vi.fn((query, params, callback) => {
          if (query.includes('SELECT totalTokens')) {
            ...
            callback(null, [
              { agentType: 'kiro', tokens: 3000, cost: 0.6, count: 6 },
              { agentType: 'cursor', tokens: 2000, cost: 0.4, count: 4 },
            ])
          } else {
            callback(null, [])
          }
    ...
    it('should export report as valid JSON', async () => {
      const mockReport = {
        ...
        statistics: {
          ...
          max: 1000,
          range: 900,
          percentiles: { p25: 250, p50: 500, p75: 750, p90: 900, p95: 950, p99: 990 },
        },
        patterns: {
          byTimeOfDay: {},
          byDayOfWeek: {},
  ...
  describe('exportAsCSV', () => {
    it('should export report as CSV with headers and data', async () => {
      const mockReport = {
        ...
        byAgent: {
          kiro: { tokens: 3000, cost: 0.03, count: 6, percentage: 60 },
        },
        byModel: {
          'claude-haiku-4.5': { tokens: 5000, cost: 0.05, count: 10, percentage: 100 },
        },
        byRequestType: {
          'spec-creation': { tokens: 5000, cost: 0.05, count: 10, percentage: 100 },
        },
        statistics: {
          mean: 500,
          median: 500,
  ...
  describe('exchangeRate', () => {
    ...
    it('should reject zero exchange rate', () => {
      expect(() => reporter.setExchangeRate(0)).toThrow('Exchange rate must be positive')
    })
  })
})
