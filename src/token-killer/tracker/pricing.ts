...
export class PricingManager {
    ...
    private userPricingOverrides: Map<
        SupportedModel,
        { input: number; output: number }
    > = new Map()
    ...
    setPricingOverride(
        ...
        this.userPricingOverrides.set(model, {
            input: inputPrice,
            output: outputPrice,
        })
    }
    ...
    async getPricing(model: SupportedModel): Promise<PricingInfo> {
        ...
        if (
            dbCached &&
            dbCached.lastUpdated.getTime() + this.cacheExpiry > Date.now()
        ) {
            this.cache.set(model, {
                data: dbCached,
                expiresAt: new Date(
                    dbCached.lastUpdated.getTime() + this.cacheExpiry
                ),
            })
            return dbCached
        }
            ...
            this.pool
                ...
                .run(
                    ...
                    [
                        ...
                        pricing.outputPrice,
                        pricing.currency,
                        pricing.lastUpdated.toISOString(),
                    ],
                    err => {
                        if (err) reject(err)
                        else resolve()
     ...
     */
    private async fetchPricingFromProvider(
        model: SupportedModel
    ): Promise<PricingInfo | null> {
        try {
    ...
    shouldPromptForPricingVerification(): boolean {
        const timeSinceLastPrompt =
            Date.now() - this.lastPricingPrompt.getTime()
     ...
     */
    static getDefaultPricing(model: SupportedModel): {
        input: number
        output: number
    } {
     ...
     */
    static getAllDefaultPricing(): Record<
        SupportedModel,
        { input: number; output: number }
    > {
