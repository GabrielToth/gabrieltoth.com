...
class ClaudeTokenizer implements Tokenizer {
    ...
    verifyTokenCount(text: string, providerCount?: number): boolean {
        ...
        const variance =
            Math.abs(calculatedCount - providerCount) /
...
class FallbackTokenizer implements Tokenizer {
    ...
    verifyTokenCount(text: string, providerCount?: number): boolean {
        ...
        const variance =
            ...
            Math.max(providerCount, 1)
        return (
            variance <= 0.15 || Math.abs(calculatedCount - providerCount) <= 1
        )
    }
...
export class TokenizerFactory {
    ...
    static getTokenizer(model: SupportedModel): Tokenizer {
        ...
        // Log fallback usage
        console.warn(
            `No specific tokenizer found for model: ${model}, using fallback`
        )
    ...
    static countInputTokens(text: string, model: SupportedModel): number {
        try {
            ...
            console.error(
                `Error counting input tokens for model ${model}:`,
                error
            )
    ...
    static countOutputTokens(text: string, model: SupportedModel): number {
        try {
            ...
            console.error(
                `Error counting output tokens for model ${model}:`,
                error
            )
            // Fallback to approximate counting
            return Math.ceil(text.length / 4)
    ...
    static verifyTokenCount(
        ...
        try {
            ...
            console.error(
                `Error verifying token count for model ${model}:`,
                error
            )
