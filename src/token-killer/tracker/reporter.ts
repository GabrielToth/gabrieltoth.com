...
import { TokenRecord, TokenReport, AgentType, SupportedModel } from '../core/types'
...
export class TokenReporter {
  ...
  async generateRequestReport(recordId: string): Promise<RequestTokenReport> {
    return new Promise((resolve, reject) => {
      ...
      this.pool.getConnection().get(query, [recordId], (err, row: any) => {
        if (err) {
          reject(err)
          return
        }
  ...
  async generateTaskReport(taskId: string): Promise<TaskTokenReport> {
    return new Promise((resolve, reject) => {
      ...
      this.pool.getConnection().all(query, [taskId], async (err, rows: any[]) => {
        if (err) {
          reject(err)
          return
        }
        ...
        if (!rows || rows.length === 0) {
          reject(new Error(`No token records found for task: ${taskId}`))
          return
        }
        ...
        try {
          ...
          const byModel: Record<SupportedModel, { tokens: number; cost: number }> = {} as any
          const byAgent: Record<AgentType, { tokens: number; cost: number }> = {} as any
      ...
      this.pool.getConnection().all(
        ...
        (err, rows: any[]) => {
          ...
          for (const row of rows) {
            ...
            if (row.agentType) {
              byAgent[row.agentType] = (byAgent[row.agentType] || 0) + (row.totalTokens || 0)
            }
            if (row.model) {
              byModel[row.model] = (byModel[row.model] || 0) + (row.totalTokens || 0)
            }
          }
   ...
   */
  async exportAsJSON(report: TokenReport | TaskTokenReport | RequestTokenReport): Promise<string> {
