...
export class AdvancedAnalyticsService {
    ...
    async generateVisualizationAnalytics(
        ...
        try {
            ...
            const [statistics, patterns, anomalies, timeSeriesData] =
                await Promise.all([
            ...
            try {
                forecast = await this.analyticsEngine.forecastConsumption(
                    startDate,
                    endDate,
                    7
                )
            ...
            const anomalyPercentage =
                timeSeriesData.length > 0
                    ? (anomalyCount / timeSeriesData.length) * 100
                    : 0

            const tokenValues = timeSeriesData.map(d => d.tokens)
            const peakTokensPerDay =
                tokenValues.length > 0 ? Math.max(...tokenValues) : 0
            const lowestTokensPerDay =
                tokenValues.length > 0 ? Math.min(...tokenValues) : 0
            const averageTokensPerDay =
                tokenValues.length > 0 ? totalTokens / tokenValues.length : 0
    ...
    async getCachedStatistics(
        ...
        const startTime = new Date()
        const data = await this.analyticsEngine.computeStatistics(
            startDate,
            endDate
        )
        const now = new Date()
    ...
    async getCachedPatterns(
        ...
        const startTime = new Date()
        const data = await this.analyticsEngine.detectPatterns(
            startDate,
            endDate
        )
        const now = new Date()
    ...
    async getCachedAnomalies(
        ...
        const startTime = new Date()
        const data = await this.analyticsEngine.detectAnomalies(
            startDate,
            endDate,
            threshold
        )
    ...
    async getCachedForecast(
        ...
        const startTime = new Date()
        const data = await this.analyticsEngine.forecastConsumption(
            startDate,
            endDate,
            forecastDays
        )
     ...
     */
    getAveragePerformanceByOperation(): Record<
        string,
        { avgDurationMs: number; count: number; cacheHitRate: number }
    > {
        const stats: Record<
            string,
