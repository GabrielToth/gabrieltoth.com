...
export class TaskBudgetManager {
  ...
  private warningCallbacks: Map<string, (status: TaskBudgetStatus) => void> = new Map()
  ...
  async allocateBudget(
    ...
    if (redThreshold <= yellowThreshold || redThreshold >= 100) {
      throw new Error('redThreshold must be between yellowThreshold and 100')
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.run(
        `INSERT INTO budget_configs (id, type, name, maxTokens, yellowThreshold, redThreshold, enabled, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [budgetId, 'task', `task-${taskId}`, maxTokens, yellowThreshold, redThreshold, 1, now.toISOString(), now.toISOString()],
        (err) => {
          if (err) {
            reject(new Error(`Failed to create task budget: ${err.message}`))
            return
          }
          ...
          connection.run(
            ...
            (usageErr) => {
              if (usageErr) {
                reject(new Error(`Failed to initialize task budget usage: ${usageErr.message}`))
                return
              }
              ...
              connection.run(
                ...
                (linkErr) => {
                  if (linkErr) {
                    reject(new Error(`Failed to link budget to task: ${linkErr.message}`))
                  } else {
                    resolve({
                      taskId,
                      budgetId,
   ...
   */
  async trackTokenConsumption(taskId: string, requestId: string, tokens: number): Promise<TaskBudgetStatus> {
    if (tokens < 0) {
      ...
      connection.get(
        ...
        (err, taskRow: any) => {
          ...
          if (!taskRow || !taskRow.budgetId) {
            reject(new Error(`Task not found or has no budget: ${taskId}`))
            return
          }
          ...
          connection.get(
            ...
            (budgetErr, budgetRow: any) => {
              if (budgetErr) {
                reject(new Error(`Failed to get budget: ${budgetErr.message}`))
                return
              }
              ...
              connection.get(
                ...
                (usageErr, usageRow: any) => {
                  if (usageErr) {
                    reject(new Error(`Failed to get budget usage: ${usageErr.message}`))
                    return
                  }

                  const currentTokens = (usageRow?.currentTokens || 0) + tokens
                  const maxTokens = budgetRow.maxTokens
                  const percentageUsed = (currentTokens / maxTokens) * 100
                  let status: 'ok' | 'warning_yellow' | 'warning_red' | 'exceeded' = 'ok'
                  let warningLevel: 'yellow' | 'red' | 'critical' | null = null

                  // Determine status
                  if (percentageUsed >= 100) {
                    status = 'exceeded'
                    warningLevel = 'critical'
                  } else if (percentageUsed >= budgetRow.redThreshold) {
                    status = 'warning_red'
                    warningLevel = 'red'
                  } else if (percentageUsed >= budgetRow.yellowThreshold) {
                  ...
                  connection.run(
                    ...
                    (updateErr) => {
                      if (updateErr) {
                        reject(new Error(`Failed to update budget usage: ${updateErr.message}`))
                        return
                      }
                      ...
                      if (warningLevel) {
                        ...
                        connection.run(
                          ...
                          (warningErr) => {
                            if (warningErr) {
                              reject(new Error(`Failed to log warning: ${warningErr.message}`))
                              return
                            }
                            ...
                            connection.get(
                              ...
                              (countErr, countRow: any) => {
                                if (countErr) {
                                  reject(new Error(`Failed to get request count: ${countErr.message}`))
                                  return
                                }

                                // Emit warning callback
                                const callback = this.warningCallbacks.get(taskId)
                                if (callback) {
                                  callback({
                                    taskId,
                                    budgetId,
   ...
   */
  async getTaskBudgetStatus(taskId: string): Promise<TaskBudgetStatus | null> {
    return new Promise((resolve, reject) => {
      const connection = this.pool.getConnection()
        ...
        (err, taskRow: any) => {
          ...
          connection.get(
            ...
            (usageErr, usageRow: any) => {
              ...
              const percentageUsed = (usageRow.currentTokens / usageRow.maxTokens) * 100
              ...
              connection.all(
                ...
                (breakdownErr, breakdownRows: any[]) => {
                  if (breakdownErr) {
                    reject(new Error(`Failed to get request breakdown: ${breakdownErr.message}`))
                    return
                  }
                  ...
                  connection.get(
                    ...
                    (countErr, countRow: any) => {
                      ...
                      resolve({
                        taskId,
                        budgetId,
                        totalTokens: usageRow.currentTokens,
                        maxTokens: usageRow.maxTokens,
                        percentageUsed,
                        status: usageRow.status,
                        requestCount: countRow?.count || 0,
                        requestBreakdown: (breakdownRows || []).map((row) => ({
                          requestId: row.id,
                          tokens: row.totalTokens,
                          timestamp: new Date(row.timestamp),
                        })),
                        lastUpdated: new Date(usageRow.lastUpdated),
                      })
                    }
                  )
                }
   ...
   */
  async generateOverageReport(taskId: string): Promise<TaskBudgetOverageReport | null> {
    return new Promise((resolve, reject) => {
      const connection = this.pool.getConnection()
        ...
        (err, taskRow: any) => {
          ...
          connection.get(
            ...
            (usageErr, usageRow: any) => {
              ...
              const overagePercentage = (overageTokens / maxTokens) * 100
              ...
              connection.all(
                ...
                (detailsErr, detailsRows: any[]) => {
                  if (detailsErr) {
                    reject(new Error(`Failed to get request details: ${detailsErr.message}`))
                    return
                  }
                  ...
                  resolve({
                    ...
                    requestDetails: (detailsRows || []).map((row) => ({
                      requestId: row.id,
                      tokens: row.totalTokens,
                      timestamp: new Date(row.timestamp),
   ...
   */
  onStatusUpdate(taskId: string, callback: (status: TaskBudgetStatus) => void): void {
   ...
   */
  async getAllTasksWithBudgets(): Promise<Array<{ taskId: string; status: TaskBudgetStatus | null }>> {
    return new Promise((resolve, reject) => {
      const connection = this.pool.getConnection()
      ...
      connection.all(
        ...
        async (err, rows: any[]) => {
          ...
          const results: Array<{ taskId: string; status: TaskBudgetStatus | null }> = []

          for (const row of rows || []) {
            try {
              const status = await this.getTaskBudgetStatus(row.id)
  ...
  async completeTask(taskId: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      connection.run(
        ...
        (err) => {
          if (err) {
            reject(new Error(`Failed to complete task: ${err.message}`))
          } else {
            resolve()
          }
        }
