         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                ...
                err => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to log threshold crossing: ${err.message}`
                            )
                        )
                    } else {
                        resolve({
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                ...
                err => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to log budget override: ${err.message}`
                            )
                        )
                    } else {
                        resolve({
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                ...
                err => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to log budget creation: ${err.message}`
                            )
                        )
                    } else {
                        resolve({
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                ...
                err => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to log budget modification: ${err.message}`
                            )
                        )
                    } else {
                        resolve({
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                ...
                err => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to log budget reset: ${err.message}`
                            )
                        )
                    } else {
                        resolve({
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.all(query, params, (err, rows: any[]) => {
                if (err) {
                    reject(
                        new Error(
                            `Failed to retrieve audit log: ${err.message}`
                        )
                    )
                } else {
                    resolve(
         ...
         ORDER BY timestamp DESC`,
                ...
                (err, rows: any[]) => {
                    if (err) {
                        reject(
                            new Error(
                                `Failed to generate audit report: ${err.message}`
                            )
                        )
                        return
                    }
                    ...
                    for (const entry of entries) {
                        ...
                        // Count by level
                        if (entry.level) {
                            eventsByLevel[entry.level] =
                                (eventsByLevel[entry.level] || 0) + 1
                        }
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.all(query, params, (err, rows: any[]) => {
                if (err) {
                    reject(
                        new Error(
                            `Failed to retrieve audit logs: ${err.message}`
                        )
                    )
                } else {
                    resolve(
