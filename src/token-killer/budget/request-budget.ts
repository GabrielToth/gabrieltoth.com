...
export class RequestBudgetManager {
  ...
  private warningCallbacks: Map<string, (warning: BudgetWarning) => void> = new Map()
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.run(
        `INSERT INTO budget_configs (id, type, name, maxTokens, yellowThreshold, redThreshold, enabled, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, 'request', name, maxTokens, yellowThreshold, redThreshold, 1, now.toISOString(), now.toISOString()],
        (err) => {
          if (err) {
            reject(new Error(`Failed to create budget: ${err.message}`))
          } else {
            ...
            connection.run(
              ...
              (usageErr) => {
                if (usageErr) {
                  reject(new Error(`Failed to initialize budget usage: ${usageErr.message}`))
                } else {
                  resolve({
                    id,
                    type: 'request',
  ...
  async getBudget(budgetId: string): Promise<BudgetConfig | null> {
    return new Promise((resolve, reject) => {
      ...
      connection.get(
        ...
        (err, row: any) => {
          if (err) {
            reject(new Error(`Failed to retrieve budget: ${err.message}`))
          } else if (row) {
            resolve({
              id: row.id,
              type: row.type,
  ...
  async getBudgetUsage(budgetId: string): Promise<BudgetUsage | null> {
    return new Promise((resolve, reject) => {
      ...
      connection.get(
        ...
        (err, row: any) => {
          if (err) {
            reject(new Error(`Failed to retrieve budget usage: ${err.message}`))
          } else if (row) {
            const percentageUsed = (row.currentTokens / row.maxTokens) * 100
            resolve({
   ...
   */
  async consumeTokens(budgetId: string, tokens: number): Promise<BudgetUsage> {
    if (tokens < 0) {
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.get(
        ...
        (err, row: any) => {
          if (err) {
            reject(new Error(`Failed to get current usage: ${err.message}`))
            return
          }

          const currentTokens = (row?.currentTokens || 0) + tokens
          const percentageUsed = (currentTokens / budget.maxTokens) * 100
              ...
              if (warningLevel) {
                ...
                connection.run(
                  ...
                  (warningErr) => {
                    ...
                    const callback = this.warningCallbacks.get(budgetId)
                    if (callback) {
                      callback({
                        id: uuidv4(),
                        budgetId,
  ...
  async overrideBudget(
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.run(
        ...
        (err) => {
          if (err) {
            reject(new Error(`Failed to override budget: ${err.message}`))
          } else {
            // Log the override
            connection.run(
              `INSERT INTO budget_warnings (id, budgetId, level, currentTokens, maxTokens, percentageUsed, message, timestamp)
              ...
              (logErr) => {
                if (logErr) {
                  reject(new Error(`Failed to log override: ${logErr.message}`))
                } else {
                  resolve({
                    ...budget,
  ...
  async resetBudget(budgetId: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      connection.run(
        ...
        (err) => {
          if (err) {
            reject(new Error(`Failed to reset budget: ${err.message}`))
          } else {
            resolve()
          }
        }
   ...
   */
  onWarning(budgetId: string, callback: (warning: BudgetWarning) => void): void {
  ...
  async getAllBudgets(): Promise<BudgetConfig[]> {
    return new Promise((resolve, reject) => {
      ...
      connection.all(
        ...
        (err, rows: any[]) => {
          if (err) {
            reject(new Error(`Failed to retrieve budgets: ${err.message}`))
          } else {
            resolve(
              (rows || []).map((row) => ({
                id: row.id,
  ...
  async setBudgetEnabled(budgetId: string, enabled: boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      connection.run(
        ...
        (err) => {
          if (err) {
            reject(new Error(`Failed to update budget: ${err.message}`))
          } else {
            resolve()
          }
        }
