...
interface ResponseSection {
  id: string
  type: 'code' | 'json' | 'yaml' | 'table' | 'reasoning' | 'prose' | 'list' | 'creative'
  content: string
  tokenCount: number
  isProtected: boolean
...
export class ResponseCompressor {
  private minSavingsPercentage = 0.20 // 20% minimum savings
  ...
  private analyzeResponse(response: string): ResponseSection[] {
    ...
    const reasoningRegex = /(?:Step \d+:|Therefore:|Thus:|In conclusion:|As a result:)[\s\S]*?(?=\n\n|Step \d+:|$)/g

    let lastIndex = 0
    const matches: Array<{ start: number; end: number; type: ResponseSection['type']; content: string }> = []
    ...
    for (const match of response.matchAll(jsonRegex)) {
      if (!this.isOverlapping(matches, match.index!, match.index! + match[0].length)) {
        matches.push({
          start: match.index!,
          end: match.index! + match[0].length,
          type: 'json',
    ...
    for (const match of matches) {
      if (match.start > lastIndex) {
        ...
        if (proseContent.trim()) {
          sections.push(this.createSection(sectionId++, 'prose', proseContent))
        }
      }

      sections.push(this.createSection(sectionId++, match.type, match.content))
      lastIndex = match.end
    }
    ...
    if (lastIndex < response.length) {
      ...
      if (remaining.trim()) {
        sections.push(this.createSection(sectionId++, 'prose', remaining))
      }
    }
   ...
   */
  private createSection(id: number, type: ResponseSection['type'], content: string): ResponseSection {
    const tokenCount = this.estimateTokenCount(content)
    const isProtected = ['code', 'json', 'yaml', 'table', 'reasoning', 'creative'].includes(type)
  ...
  private detectCreativeContent(sections: ResponseSection[]): void {
    for (const section of sections) {
      if (section.type === 'prose') {
        ...
        if (content.includes('imagine') || content.includes('story') || 
            content.includes('poem') || content.includes('creative') ||
            content.includes('metaphor') || content.includes('analogy')) {
  ...
  private calculateReadabilityScore(sections: ResponseSection[]): number {
    ...
    const hasStructured = sections.some(s => ['json', 'yaml', 'table'].includes(s.type))
  ...
  public compress(response: string, dryRun = false): CompressionResult {
    ...
    const savingsPercentage = originalTokens > 0 ? savings / originalTokens : 0

    const readabilityScore = this.calculateReadabilityScore(compressed)

    // Check if compression would significantly reduce clarity
    if (readabilityScore < 0.5 || savingsPercentage < this.minSavingsPercentage) {
