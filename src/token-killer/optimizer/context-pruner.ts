...
interface ContextSegment {
  id: string
  type: 'system_prompt' | 'user_query' | 'critical_instruction' | 'task_context' | 
        'old_conversation' | 'duplicate' | 'metadata' | 'example' | 'summary'
  content: string
  tokenCount: number
...
export class ContextPruner {
  ...
  private analyzeContext(context: string): ContextSegment[] {
    ...
    for (const line of lines) {
      ...
      if (line.includes('system:') || line.includes('System:')) {
        if (currentSegment) {
          segments.push(this.createSegment(segmentId++, currentType, currentSegment))
        }
      ...
      // Detect task context
      if (line.includes('task:') || line.includes('Task:') || line.includes('context:')) {
        if (currentSegment) {
          segments.push(this.createSegment(segmentId++, currentType, currentSegment))
        }
   ...
   */
  private createSegment(id: number, type: ContextSegment['type'], content: string): ContextSegment {
  ...
  private detectDuplicates(segments: ContextSegment[]): void {
    ...
    for (const segment of segments) {
      ...
      if (seen.has(normalized) && segment.type !== 'system_prompt' && 
          segment.type !== 'critical_instruction' && segment.type !== 'task_context') {
  ...
  private detectOldConversations(segments: ContextSegment[]): void {
    ...
    for (const segment of segments) {
      if (segment.type === 'user_query' || segment.content.includes('User:') || 
          segment.content.includes('Assistant:')) {
        turnCount++
      }
      
      if (turnCount > this.oldConversationThreshold && 
  ...
  private detectExamplesAndSummaries(segments: ContextSegment[]): void {
    for (const segment of segments) {
      ...
      if ((content.includes('example:') || content.includes('e.g.') || 
           content.includes('for example')) && segment.priority > 3) {
        segment.type = 'example'
        segment.priority = 3
      }
      
      if ((content.includes('summary:') || content.includes('summarize') ||
           content.includes('in summary')) && segment.priority > 4) {
   ...
   */
  private pruneSegments(segments: ContextSegment[], targetSavingsPercentage: number): {
    remaining: ContextSegment[]
    removed: ContextSegment[]
  } {
    const totalTokens = segments.reduce((sum, seg) => sum + seg.tokenCount, 0)
    const targetTokensSaved = Math.ceil(totalTokens * targetSavingsPercentage)
  ...
  private calculateSemanticCoherence(remaining: ContextSegment[]): number {
    ...
    const hasSystemPrompt = remaining.some(seg => seg.type === 'system_prompt')
  ...
  public prune(context: string, dryRun = false): PruningResult {
    ...
    const prunedTokens = remaining.reduce((sum, seg) => sum + seg.tokenCount, 0)
    const tokensSaved = originalTokens - prunedTokens
    const savingsPercentage = originalTokens > 0 ? tokensSaved / originalTokens : 0

    // Verify we achieved minimum savings
    if (savingsPercentage < this.minSavingsPercentage && removed.length > 0) {
      ...
      for (const segment of remaining) {
        if (segment.priority < 5 && additionalRemoved < additionalNeeded) {
          removed.push(segment)
    ...
    const finalPrunedTokens = finalRemaining.reduce((sum, seg) => sum + seg.tokenCount, 0)
    const finalTokensSaved = originalTokens - finalPrunedTokens
    const finalSavingsPercentage = originalTokens > 0 ? finalTokensSaved / originalTokens : 0

    const semanticCoherence = this.calculateSemanticCoherence(finalRemaining)
