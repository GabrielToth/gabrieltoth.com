...
export class DryRunEngine {
  ...
  public executeDryRun(
    ...
    for (const strategy of enabledStrategies) {
      ...
      if (strategy.type === 'pruning' && context) {
        ...
        strategyResult.estimatedTokensSaved = result.estimatedTokensSaved
        strategyResult.estimatedSavingsPercentage = result.estimatedSavingsPercentage
        ...
        strategyResult.estimatedSavingsPercentage = strategy.estimatedSavings
        strategyResult.resultingTokens = currentTokens - estimatedSavings
    ...
    const estimatedSavingsPercentage = originalTokens > 0 ? estimatedTokensSaved / originalTokens : 0
  ...
  public validateAccuracy(
    ...
    const differencePercentage = dryRunResult.originalTokens > 0 
    ...
    if (!isAccurate) {
      ...
      recommendations.push('Dry-run accuracy is within acceptable threshold.')
    }
  ...
  public generateReport(dryRunResult: DryRunExecutionResult): string {
    ...
    lines.push('SUMMARY')
    lines.push('-'.repeat(60))
    lines.push(`Original Tokens:        ${dryRunResult.originalTokens.toLocaleString()}`)
    lines.push(`Estimated After:        ${dryRunResult.estimatedTokensAfter.toLocaleString()}`)
    lines.push(`Estimated Savings:      ${dryRunResult.estimatedTokensSaved.toLocaleString()} tokens`)
    lines.push(`Savings Percentage:     ${(dryRunResult.estimatedSavingsPercentage * 100).toFixed(2)}%`)
    lines.push(`Estimated Accuracy:     ${(dryRunResult.accuracy * 100).toFixed(1)}%`)
    ...
    for (const strategy of dryRunResult.strategies) {
      ...
      lines.push(`  Original Tokens:      ${strategy.originalTokens.toLocaleString()}`)
      lines.push(`  Resulting Tokens:     ${strategy.resultingTokens.toLocaleString()}`)
      lines.push(`  Estimated Savings:    ${strategy.estimatedTokensSaved.toLocaleString()} tokens`)
      lines.push(`  Savings Percentage:   ${(strategy.estimatedSavingsPercentage * 100).toFixed(2)}%`)
      lines.push('')
    }
   ...
   */
  public generateValidationReport(validation: DryRunValidationResult): string {
    const lines: string[] = []
    ...
    lines.push('-'.repeat(60))
    lines.push(`Status:                 ${validation.isAccurate ? '✓ ACCURATE' : '✗ INACCURATE'}`)
    lines.push(`Accuracy Percentage:    ${(validation.accuracyPercentage * 100).toFixed(2)}%`)
    lines.push(`Within Threshold:       ${validation.withinThreshold ? 'Yes' : 'No'} (${(validation.threshold * 100).toFixed(1)}%)`)
    lines.push('')

    lines.push('COMPARISON')
    lines.push('-'.repeat(60))
    lines.push(`Estimated Tokens:       ${validation.estimatedTokens.toLocaleString()}`)
    lines.push(`Actual Tokens:          ${validation.actualTokens.toLocaleString()}`)
    lines.push(`Difference:             ${validation.difference.toLocaleString()} tokens`)
    lines.push(`Difference Percentage:  ${(validation.differencePercentage * 100).toFixed(2)}%`)
    lines.push('')
