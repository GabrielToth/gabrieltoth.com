...
import type { AggregatedTokenData, TimeWindow, TokenBreakdownResponse } from './types'
  ...
  useEffect(() => {
    ...
    const fetchAndRenderChart = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch breakdown data: ${response.statusText}`)
        }

        const breakdownData: TokenBreakdownResponse = await response.json()

        // Prepare data for chart
        const categories = breakdownData.breakdown.map((item) => item.category)
        const tokens = breakdownData.breakdown.map((item) => item.totalTokens)
