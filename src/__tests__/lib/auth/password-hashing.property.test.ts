    ...
    it("should handle edge cases in password comparison", async () => {
        await fc.assert(
            fc.asyncProperty(validPasswordArb, async password => {
                ...
                expect(await comparePassword(password + "x", hash)).toBe(false)
    ...
    it("should handle invalid inputs gracefully", async () => {
        await fc.assert(
            fc.asyncProperty(validPasswordArb, async password => {
                ...
                expect(await comparePassword(password, undefined as any)).toBe(
                    false
                )
                ...
                expect(await comparePassword(password, "invalid-hash")).toBe(
                    false
                )
            }),
            { numRuns: 30 }
