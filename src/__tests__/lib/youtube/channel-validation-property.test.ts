...
describe("Channel Validation Service - Property-Based Tests", () => {
    ...
    describe("Property 1: Channel ID Validation", () => {
        it("should validate matching channel IDs correctly", async () => {
            await fc.assert(
        ...
        it("should reject mismatched channel IDs", async () => {
            await fc.assert(
                fc.asyncProperty(
    ...
    describe("Property 2: Channel Info Extraction", () => {
        it("should extract all channel info fields correctly", async () => {
            await fc.assert(
                fc.asyncProperty(youtubeChannelInfoArb, async channelInfo => {
                    const mockResponse = {
                        items: [
                            {
                                ...
                                snippet: {
                                    ...
                                    thumbnails: channelInfo.profileImageUrl
                                        ? {
                                              default: {
                    ...
                    const result = await service.getChannelInfo("test-token")

                    // Property: All channel info fields should be extracted correctly
                    expect(result?.channelId).toBe(channelInfo.channelId)
                    expect(result?.title).toBe(channelInfo.title)
                    expect(result?.description).toBe(channelInfo.description)
                    expect(result?.customUrl).toBe(channelInfo.customUrl)
    ...
    describe("Property 3: Error Handling", () => {
        it("should handle invalid access tokens consistently", async () => {
            await fc.assert(
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async requestCount => {
                        ...
                        expect(successCount).toBe(Math.min(requestCount, 100))
    ...
    describe("Property 5: Validation Result Consistency", () => {
        it("should return consistent validation results for same inputs", async () => {
            await fc.assert(
                fc.asyncProperty(youtubeChannelIdArb, async channelId => {
                    const oauthResponse: OAuthResponse = {
                        accessToken: "test-token",
