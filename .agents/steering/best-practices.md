...
### 1.1 No task/phase labels in code (MANDATORY)

Do **not** embed implementation-plan metadata in source, tests, or docs:

- **Forbidden in code/tests**: `Task 8.3`, `Phase 10`, `PHASE_1`, numbered spec checklists in comments, file names like `phase-10-*.test.ts`
- **Use instead**: behavior-focused names (`password-storage-security.test.ts`, `describe("rate limiting")`, `// Secure cookie settings`)
- **Test steps**: use `Step 1`, `Step 2` only for multi-step test scenarios — never `Phase 1` for project milestones
- **Specs**: keep phase/task breakdowns in issue trackers or PR descriptions, not in the repo tree
